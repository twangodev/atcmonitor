...
def main():
    for message in consumer:
        ...
        if inside_any:
            ...
            inside_wider = any(point.within(poly) for poly in polygonsGeneral + polygonsLaser)
            if not inside_wider:
                kafkaMessage["rating"] = 1
                kafkaMessage["message"] = "Outside wider transition areas"

        # Send alert if needed
        if kafkaMessage["rating"] != 0:
            print(kafkaMessage)
