        ...
        if alt is not None and alt is not '' and float(alt) <= 10000:
            ...
            if alt < 1000 and inside_any:
                if inside_precision:
                    ...
                    kafkaMessage["message"] = f"Outside 3D laser-thin final approach and wider transition areas "
            else:
                if inside_precision:
                    kafkaMessage["rating"] = 0  
            ...
            if inside_any:
                ...
                if not (inside_precision or inside_wider):
                    ...
                    kafkaMessage["rating"] = 4
                    kafkaMessage["message"] = f"Outside laser-thin final approach and wider transition areas"
                elif not(inside_precision):
                    kafkaMessage["rating"] = 3
                    kafkaMessage["message"] = f"Outside laser-thin final approach"

            else:
                # Outside airport buffer altogether: check both general + precision zones
                if not (inside_wider or inside_precision):
                    kafkaMessage["rating"] = 2
                    kafkaMessage["message"] = f"Outside wider transition and laser-thin areas outside final approach"
                elif not (inside_precision):
                    kafkaMessage["rating"] = 1
                    kafkaMessage["message"] = f"Inside wider transition areas outside final approach"
                else:
                    kafkaMessage["rating"] = 0
                    kafkaMessage["message"] = f"Inside wider transition and laser-thin areas outside final approach"
